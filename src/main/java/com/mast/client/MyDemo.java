...
public class MyDemo extends JPanel {
    ...
    public static Mat detectFace(Mat img) {
        ...
        CascadeClassifier eyeDetector = new CascadeClassifier("/usr/local/share/OpenCV/haarcascades/haarcascade_eye.xml");
        ...
        if(rects != null && rects.length >= 1){
            for (Rect rect : rects) {
                ...
                //画矩形
                Imgproc.rectangle(img, new Point(rect.x - 20, rect.y - 40), new Point(rect.x + rect.width + 20, rect.y + rect.height + 20),
...
//                        new Scalar(0, 0, 255), 2);
                ...
                for (Rect eyeRect : eyeRects) {
                    //画矩形
                    Imgproc.rectangle(img, new Point(rect.x + eyeRect.x, rect.y + eyeRect.y), new Point(rect.x + eyeRect.x + eyeRect.width,rect.y + eyeRect.y + eyeRect.height),
                            new Scalar(0, 0, 255), 2);
                }
                if( eyesDetections.toArray().length > 1){
//                    save(img, rect, "D:\\Pics\\root"+r.nextInt(2000)+".jpg");
                }
